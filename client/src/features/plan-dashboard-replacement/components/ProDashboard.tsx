...
function ProDashboard({ activeTab, theme, filters, onFiltersChange }: Props) {
  ...
  const segmentedNoShow = useMemo(() => {
    const byDoctor = byProf.map(p => ({ segment: `MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©dico: ${p.name}`, noShowRate: p.noShowRate, n: p.total }));
    ...
    const slots = ['09h', '11h', '14h', '16h'].map((slot, idx) => ({
      segment: `HorÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡rio: ${slot}`,
  ...
  const slotOccupancyByProf = useMemo(() => {
    return byProf.flatMap((p, pIdx) => ['09h','11h','14h','16h'].map((slot, sIdx) => {
      ...
      return { label: `${p.name} ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢à¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ ${slot}`, prof: p.name, slot, occupancy, capacityN: Math.max(4, Math.round(p.total/4)) };
    }));
  }, [byProf]);
  ...
  const cancelReasons = useMemo(() => {
    ...
    const rows = [
      { reason: 'PreÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§o / orÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§amento', count: Math.round(totalCancels * 0.26) },
      { reason: 'Conflito de agenda', count: Math.round(totalCancels * 0.22) },
      { reason: 'Melhorou / desistiu', count: Math.round(totalCancels * 0.18) },
      { reason: 'Sem confirmaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o', count: Math.round(totalCancels * 0.16) },
      { reason: 'LocalizaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o / transporte', count: Math.max(1, totalCancels - (Math.round(totalCancels * 0.26)+Math.round(totalCancels * 0.22)+Math.round(totalCancels * 0.18)+Math.round(totalCancels * 0.16))) },
    ];
  ...
  const agendaProRules = useMemo(() => {
    ...
    const rows: Array<{ id: string; kpi: string; value: string; meta: string; baseN: string; priority: Priority; action: string }> = [
      ...
      { id:'02', kpi:'OcupaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o por Profissional/Slot (%)', value:`${(worstSlot?.occupancy ?? 0).toFixed(0)}%`, meta:'> 80%', baseN:String(worstSlot?.capacityN ?? 0), priority:classifyOcc(worstSlot?.occupancy ?? 0), action:'RedistribuiÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o automÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡tica de slots' },
      { id:'03', kpi:'Heatmap DiaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬à¢Ã¢â€šÂ¬Ã‚ÂHoraÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬à¢Ã¢â€šÂ¬Ã‚ÂProf', value:'PadrÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o recorrente detectado', meta:'Nenhum slot <40% apÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³s 10h', baseN:String(kpis.total), priority:((heatmapDayHourProf.some(r=>r.data.some(c=>c.x!=='08h' && Number(c.y) < 40))) ? 'P2' : 'OK'), action:'Filtro por especialidade + redistribuiÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o' },
      { id:'04', kpi:'Lead Time do Agendamento (dias)', value:`${((current?.leadTimeDays ?? 0)).toFixed(1)}d`, meta:'< 3d', baseN:String(kpis.leads), priority:classifyLead(current?.leadTimeDays ?? 0), action:'Atuar no funil comercial/recepÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o' },
      { id:'05', kpi:'Cancelamentos por Motivo', value:`${(topCancel?.pct ?? 0).toFixed(0)}%`, meta:'Top motivo < 30%', baseN:String(kpis.canceled), priority:classifyCancel(topCancel?.pct ?? 0), action:`AÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o estrutural: ${topCancel?.reason ?? '-'}` },
  ...
  const financeAdvRules = useMemo(() => {
    ...
    return [
      ...
      { id:'05', kpi:'Recebiveis Aging', value:`>90d ${gt90PctRevenue.toFixed(1)}%`, meta:'>60d < 5% receita', baseN:fmt(agingReceivables.totalRecv), priority:(gt90PctRevenue > 5 ? 'P1' : 'OK') as Priority, action:'CobranÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§a automatica WhatsApp + rÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©gua' },
  ...
  return (
    <div className="animate-fade-in" key={activeTab}>
      ...
      {/* ===== VISÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â à¢Ã¢â€šÂ¬Ã¢â€žÂ¢O CEO PRO ===== */}
      {activeTab === 0 && (<>
        <div className="section-header"><h2><span className="orange-bar" /> VisÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o CEO ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢à¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â Pro</h2></div>
        <div className="overview-row">
          <div className="overview-card"><div className="overview-card-label">Receita Bruta</div><div className="overview-card-value">{fmt(kpis.grossRevenue)}</div><div className="overview-card-info"><div className="dot" style={{background:'var(--green)'}}/><span>PerÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­odo</span></div></div>
          ...
          <div className="overview-card"><div className="overview-card-label">OcupaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o</div><div className="overview-card-value">{kpis.occupancyRate.toFixed(1)}%</div></div>
        </div>
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">Receita Semanal (Bruto ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬à¢Ã¢â€šÂ¬Ã‚Â LÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­quido)</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'area'},fill:{type:'gradient',gradient:{shadeIntensity:1,opacityFrom:0.4,opacityTo:0}},colors:['#ff5a1f','#45a29e'],xaxis:{...ct.xaxis,categories:weeklyTrend.map(w=>w.label)},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:'Bruto',data:weeklyTrend.map(w=>Math.round(w.grossRevenue))},{name:'LÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­quido',data:weeklyTrend.map(w=>Math.round(w.netRevenue))}]} type="area" height={230}/>
          </div></div>
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">OcupaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o Semanal</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{curve:'smooth',width:3},colors:['#ff5a1f'],markers:{size:4},annotations:{yaxis:[{y:75,borderColor:'#22c55e',strokeDashArray:4,label:{text:'Meta 75%',style:{color:'#fff',background:'#22c55e'}}}]},xaxis:{...ct.xaxis,categories:weeklyTrend.map(w=>w.label)}}} series={[{name:'OcupaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o %',data:weeklyTrend.map(w=>+w.occupancyRate.toFixed(1))}]} type="line" height={230}/>
          </div></div>
        </div>
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">Score por ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Ârea</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{type:'radar'},xaxis:{categories:['Agenda','Financeiro','Marketing','OperaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o','ExperiÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âªncia']},yaxis:{show:false},colors:['#ff5a1f']}} series={[{name:'Score',data:[Math.round(kpis.occupancyRate/10),Math.round(kpis.margin*5/20),Math.round(kpis.leads/40),Math.round(kpis.avgNPS),Math.round(kpis.returnRate/5)]}]} type="radar" height={230}/>
          </div></div>
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">Indicadores SemÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡foro</span></div><div className="chart-card-body" style={{padding:14}}>
            <table className="data-table"><thead><tr><th>Indicador</th><th>Valor</th><th>Meta</th><th>Status</th></tr></thead><tbody>
              <tr><td>No-Show</td><td style={{fontWeight:700}}>{kpis.noShowRate.toFixed(1)}%</td><td>{'<'}10%</td><td><span className={`chart-card-badge ${kpis.noShowRate<=10?'green':kpis.noShowRate<=15?'yellow':'red'}`} style={{display:'inline-block'}}>{kpis.noShowRate<=10?'OK':kpis.noShowRate<=15?'ATENÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â à¢Ã¢â€šÂ¬Ã¢â€žÂ¢O':'CRÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂTICO'}</span></td></tr>
              <tr><td>OcupaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o</td><td style={{fontWeight:700}}>{kpis.occupancyRate.toFixed(1)}%</td><td>{'>'}75%</td><td><span className={`chart-card-badge ${kpis.occupancyRate>=75?'green':'yellow'}`} style={{display:'inline-block'}}>{kpis.occupancyRate>=75?'OK':'ATENÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â à¢Ã¢â€šÂ¬Ã¢â€žÂ¢O'}</span></td></tr>
              <tr><td>NPS</td><td style={{fontWeight:700}}>{kpis.avgNPS.toFixed(1)}</td><td>{'>'}8.0</td><td><span className={`chart-card-badge ${kpis.avgNPS>=8?'green':'yellow'}`} style={{display:'inline-block'}}>{kpis.avgNPS>=8?'OK':'ATENÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â à¢Ã¢â€šÂ¬Ã¢â€žÂ¢O'}</span></td></tr>
              <tr><td>Margem</td><td style={{fontWeight:700}}>{kpis.margin.toFixed(1)}%</td><td>{'>'}20%</td><td><span className={`chart-card-badge ${kpis.margin>=20?'green':'red'}`} style={{display:'inline-block'}}>{kpis.margin>=20?'OK':'CRÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂTICO'}</span></td></tr>
              <tr><td>CAC</td><td style={{fontWeight:700}}>{fmt(kpis.avgCAC)}</td><td>{'<'}R$150</td><td><span className={`chart-card-badge ${kpis.avgCAC<=150?'green':'red'}`} style={{display:'inline-block'}}>{kpis.avgCAC<=150?'OK':'CRÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚ÂTICO'}</span></td></tr>
              <tr><td>Ticket MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©dio</td><td style={{fontWeight:700}}>{fmt(kpis.avgTicket)}</td><td>{'>'}R$300</td><td><span className={`chart-card-badge ${kpis.avgTicket>=300?'green':'yellow'}`} style={{display:'inline-block'}}>{kpis.avgTicket>=300?'OK':'ATENÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â à¢Ã¢â€šÂ¬Ã¢â€žÂ¢O'}</span></td></tr>
            </tbody></table>
          </div></div>
        </div>
      </>)}
      ...
      {/* ===== WAR ROOM ===== */}
      {activeTab === 1 && (<>
        <div className="section-header"><h2><span className="orange-bar" /> War Room ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢à¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â Alertas e AÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o</h2></div>
        <div className="overview-row">
          ...
          <div className="overview-card"><div className="overview-card-label">Espera MÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©dia</div><div className="overview-card-value" style={{color:kpis.avgWait>15?'var(--yellow)':'var(--green)'}}>{kpis.avgWait.toFixed(0)} min</div></div>
        ...
        <div className="chart-grid">
          ...
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">Heatmap Espera (Prof ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬à¢Ã¢â€šÂ¬Ã‚Â Dia)</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{type:'heatmap'},colors:['#ff5a1f'],dataLabels:{enabled:true},plotOptions:{heatmap:{shadeIntensity:0.5,colorScale:{ranges:[{from:0,to:15,color:'#22c55e',name:'OK'},{from:16,to:25,color:'#eab308',name:'AtenÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o'},{from:26,to:60,color:'#ef4444',name:'CrÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­tico'}]}}}}} series={byProf.map(p=>({name:p.name,data:byWeekday.map(w=>({x:w.name,y:Math.round(w.avgWait+(p.avgWait-kpis.avgWait)*0.5)}))}))} type="heatmap" height={200}/>
          </div></div>
        </div>
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">No-Show por Profissional</span><span style={{fontSize:10,color:'var(--text-muted)'}}>ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â°ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Clique</span></div><div className="chart-card-body">
          ...
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">Alertas Ativos</span></div><div className="chart-card-body" style={{padding:14}}>
            {kpis.noShowRate>10 && <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}><span className="chart-card-badge red" style={{display:'inline-block'}}>P1</span><span style={{color:'var(--text-secondary)',fontSize:12}}>No-Show Rate {kpis.noShowRate.toFixed(1)}% ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢à¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â acima da meta</span></div>}
            {kpis.avgCAC>150 && <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}><span className="chart-card-badge red" style={{display:'inline-block'}}>P1</span><span style={{color:'var(--text-secondary)',fontSize:12}}>CAC {fmt(kpis.avgCAC)} ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢à¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â acima do teto R$150</span></div>}
            {kpis.avgWait>15 && <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}><span className="chart-card-badge yellow" style={{display:'inline-block'}}>P2</span><span style={{color:'var(--text-secondary)',fontSize:12}}>Espera {kpis.avgWait.toFixed(0)}min ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢à¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â meta {'<'}15min</span></div>}
            {kpis.avgNPS<8 && <div style={{display:'flex',alignItems:'center',gap:8}}><span className="chart-card-badge yellow" style={{display:'inline-block'}}>P2</span><span style={{color:'var(--text-secondary)',fontSize:12}}>NPS {kpis.avgNPS.toFixed(1)} ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢à¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â abaixo da meta 8.0</span></div>}
          </div></div>
        </div>
      </>)}
      ...
      {/* ===== AGENDA / OTIMIZAÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â à¢Ã¢â€šÂ¬Ã¢â€žÂ¢O ===== */}
      {activeTab === 3 && (<>
        <div className="section-header"><h2><span className="orange-bar" /> Agenda / OtimizaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o</h2></div>
        <div className="chart-card" style={{marginBottom: 16}}>
        ...
        <div className="overview-row">
          ...
          <div className="overview-card"><div className="overview-card-label">OcupaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o</div><div className="overview-card-value">{kpis.occupancyRate.toFixed(1)}%</div></div>
        ...
        <div className="chart-grid">
          ...
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">02 OcupaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o por Profissional/Slot (%)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Slots ociosos e redistribuiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{type:'bar'},plotOptions:{bar:{horizontal:true,distributed:true,barHeight:'58%',borderRadius:4}},colors:slotOccupancyByProf.map(s=>s.occupancy<55?'#ef4444':s.occupancy<70?'#eab308':s.occupancy<80?'#3b82f6':'#22c55e'),xaxis:{...ct.xaxis,max:100},annotations:{xaxis:[{x:80,borderColor:'#22c55e',strokeDashArray:4,label:{text:'Meta 80%',style:{color:'#fff',background:'#22c55e'}}}]},legend:{show:false}}} series={[{name:'OcupaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o %',data:slotOccupancyByProf.map(s=>({x:s.label,y:+s.occupancy.toFixed(1)}))}]} type="bar" height={230}/>
          </div></div>
        </div>
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">03 Heatmap Dia ÃƒÆ’Ã†â€™à¢Ã¢â€šÂ¬Ã¢â‚¬Â Hora ÃƒÆ’Ã†â€™à¢Ã¢â€šÂ¬Ã¢â‚¬Â Profissional</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Heatmap multi-dimensional interativo</span></div><div className="chart-card-body">
        ...
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">05 Cancelamentos por Motivo</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Barra ordenada por frequÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âªncia</span></div><div className="chart-card-body">
        ...
        <div className="detail-section"><div className="detail-section-header">ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸à¢Ã¢â€šÂ¬Ã…â€œà¢Ã¢â€šÂ¬Ã‚Â¹ Regras de NegÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³cio - Agenda Pro</div><div className="detail-section-body"><table className="data-table"><thead><tr><th>ID</th><th>KPI</th><th>Valor</th><th>Meta</th><th>Base N</th><th>Status</th><th>AÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o</th></tr></thead><tbody>
      ...
      {/* ===== INTEGRAÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢ES ===== */}
      {activeTab === 5 && (<>
        <div className="section-header"><h2><span className="orange-bar" /> IntegraÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âµes</h2></div>
        <div className="overview-row">
          ...
          <div className="overview-card"><div className="overview-card-label">ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ltima Sync</div><div className="overview-card-value" style={{fontSize:16}}>HÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ 5 min</div></div>
        ...
        <div className="detail-section"><div className="detail-section-header">ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â°ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡ Status das IntegraÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âµes</div><div className="detail-section-body"><table className="data-table"><thead><tr><th>Sistema</th><th>Status</th><th>ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ltima Sync</th><th>Registros</th></tr></thead><tbody>
      ...
      {activeTab === 7 && (<>
        ...
        <div className="detail-section"><div className="detail-section-header">ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â°ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¹Ã…â€œÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¥ Performance da Equipe</div><div className="detail-section-body"><table className="data-table"><thead><tr><th>Profissional</th><th>Consultas</th><th>Receita</th><th>Ticket</th><th>NPS</th><th>No-Show</th><th>OcupaÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â£o</th><th>Espera</th></tr></thead><tbody>
        ...
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">Ranking Receita</span><span style={{fontSize:10,color:'var(--text-muted)'}}>ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â°ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¸ÃƒÆ’Ã‚Â¢à¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Clique</span></div><div className="chart-card-body">
