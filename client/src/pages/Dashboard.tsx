...
.D{background-color:var(--bg);background-image:radial-gradient(var(--bd) 1px,transparent 1px);background-size:24px 24px;color:var(--tp);font-family:'Roboto',sans-serif;min-height:100vh;overflow-x:hidden;-webkit-font-smoothing:antialiased;transition:background-color .3s,color .3s;display:flex;animation:fi .5s}
...
export default function Dashboard() {
  ...
  return (<><style>{CSS}</style><div className={`D ${lt ? "lt" : ""}`}>
    ...
    {/* MAIN */}
    <main className="mn">
      <header className="tb"><div className="tb-t">{titles[scr] || "Workspace"}</div><div style={{ display: "flex", gap: 16, alignItems: "center" }}><select className="fs" value={plan} onChange={e => setPlan(e.target.value)} style={{ margin: 0, width: "auto", height: 36, padding: "4px 12px", fontWeight: "bold" }}><option value="Essential">Plano: Essential</option><option value="Pro">Plano: Pro</option><option value="Enterprise">Plano: Enterprise</option></select><button className="bt bp" onClick={() => setPdf(true)}>Exportar PDF</button><button className="bt bgh" onClick={() => setLt(!lt)} style={{ fontSize: 16 }}>☀️ / 🌙</button></div></header>
