...
function EssentialDashboard({ activeTab, theme, filters, onFiltersChange }: Props) {
  ...
  const agendaRules = useMemo(() => {
    ...
    return [
      ...
      { id: '02', kpi: 'Taxa de OcupaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o (%)', value: `${occupancy.toFixed(1)}%`, meta: '> 75%', baseN: `${current?.total ?? kpis.total}`, priority: classifyOccupancy(occupancy), action: 'Ajustar capacidade por profissional/turno' },
      { id: '03', kpi: 'ConfirmaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âµes Realizadas (%)', value: `${confirmations.toFixed(1)}%`, meta: '> 80%', baseN: `${current?.total ?? kpis.total}`, priority: classifyConfirm(confirmations), action: 'Automatizar confirmaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âµes (WhatsApp)' },
      ...
      { id: '07', kpi: 'Lead Time do Agendamento (dias)', value: `${leadTime.toFixed(1)}d`, meta: '< 3d', baseN: `${kpis.leads}`, priority: classifyLead(leadTime), action: 'Atuar na recepÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o se > 7d' },
    ];
  ...
  const financeRules = useMemo(() => {
    ...
    return [
      ...
      { id:'02', kpi:'Receita LÃƒÆ’Ã‚Â­quida', value: `${current.netPctGross.toFixed(1)}% do bruto`, meta:'> 92% do bruto', baseN:fmt(current.gross), priority:netPriority, action:'Monitorar glosas, estornos e inadimplÃƒÆ’Ã‚Âªncia' },
      { id:'03', kpi:'Margem LÃƒÆ’Ã‚Â­quida (%)', value: `${current.marginPct.toFixed(1)}%`, meta:'> 18%', baseN:fmt(current.net), priority:marginPriority, action:'Comparar benchmark da especialidade' },
      { id:'04', kpi:'Ticket MÃƒÆ’Ã‚Â©dio (R$)', value: fmt(current.ticketAvg), meta: fmt(current.ticketBenchmark), baseN:String(current.consultations), priority:ticketPriority, action:'Segmentar por procedimento' },
      { id:'05', kpi:'InadimplÃƒÆ’Ã‚Âªncia (%)', value: `${current.delinquencyPct.toFixed(1)}%`, meta:'< 5%', baseN:fmt(current.gross), priority:inadPriority, action:'CobranÃƒÆ’Ã‚Â§a ativa e polÃƒÆ’Ã‚Â­tica de recebÃƒÆ’Ã‚Â­veis' },
  ...
  const marketingRules = useMemo(() => {
    ...
    return [
      { id:'01', kpi:'Leads Gerados / Semana', value:String(current.leads), meta:String(current.leadMeta), baseN:String(current.leads), priority:leadsPriority, action:'Comparar semana anterior e alertar tendÃƒÂªncia' },
      { id:'02', kpi:'ConversÃƒÂ£o Lead Ã¢â€ â€™ Agendamento (%)', value:`${current.conversion.toFixed(1)}%`, meta:'> 30%', baseN:String(current.leads), priority:conversionPriority, action:'Separar funil por canal (script quebrando)' },
      { id:'03', kpi:'No-Show por Canal de Origem (%)', value:`${(worstNoShowChannel.noShowRateChannel ?? 0).toFixed(1)}% (${(worstNoShowChannel as any).name ?? '-'})`, meta:'< mÃƒÂ©dia geral', baseN:String((worstNoShowChannel as any).total ?? 0), priority:noShowPriority, action:'Atacar canal com no-show acima da mÃƒÂ©dia' },
  ...
  return (
    <div className="animate-fade-in" key={activeTab}>
      ...
      {/* ===== VISÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢O CEO ===== */}
      {activeTab === 0 && (<>
        <div className="section-header"><h2><span className="orange-bar" /> {t('VisÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o CEO ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Resumo')}</h2></div>
        <div className="overview-row">
          <div className="overview-card"><div className="overview-card-label">{t('Faturamento Bruto')}</div><div className="overview-card-value">{fmt(kpis.grossRevenue)}</div><div className="overview-card-info"><div className="dot" style={{background:'var(--green)'}}/><span>{t('PerÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­odo')}</span></div></div>
          <div className="overview-card"><div className="overview-card-label">{t('Receita LÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­quida')}</div><div className="overview-card-value">{fmt(kpis.netRevenue)}</div><div className="overview-card-info"><div className="dot" style={{background:'var(--green)'}}/><span>{t('92% do bruto')}</span></div></div>
          <div className="overview-card"><div className="overview-card-label">{t('OcupaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o')}</div><div className="overview-card-value">{kpis.occupancyRate.toFixed(1)}%</div></div>
        ...
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">{t('EvoluÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o Receita')}</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'area'},fill:{type:'gradient',gradient:{shadeIntensity:1,opacityFrom:0.4,opacityTo:0}},colors:['#ff5a1f','#45a29e'],xaxis:{...ct.xaxis,categories:weeklyTrend.map(w=>w.label)},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:t('Bruto'),data:weeklyTrend.map(w=>Math.round(w.grossRevenue))},{name:t('LÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­quido'),data:weeklyTrend.map(w=>Math.round(w.netRevenue))}]} type="area" height={220}/>
          </div></div>
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">{t('Indicadores SemÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡foro')}</span></div><div className="chart-card-body" style={{padding:18}}>
            <table className="data-table"><thead><tr><th>{t('Indicador')}</th><th>{t('Valor')}</th><th>{t('Meta')}</th><th>{t('Status')}</th></tr></thead><tbody>
              <tr><td>{t('No-Show')}</td><td style={{fontWeight:700}}>{kpis.noShowRate.toFixed(1)}%</td><td>{'<'} 10%</td><td><span className={`chart-card-badge ${kpis.noShowRate<=10?'green':kpis.noShowRate<=15?'yellow':'red'}`} style={{display:'inline-block'}}>{kpis.noShowRate<=10?t('OK'):kpis.noShowRate<=15?t('ATENÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢O'):t('CRÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂTICO')}</span></td></tr>
              <tr><td>{t('OcupaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o')}</td><td style={{fontWeight:700}}>{kpis.occupancyRate.toFixed(1)}%</td><td>{'>'} 75%</td><td><span className={`chart-card-badge ${kpis.occupancyRate>=75?'green':'yellow'}`} style={{display:'inline-block'}}>{kpis.occupancyRate>=75?t('OK'):t('ATENÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢O')}</span></td></tr>
              <tr><td>{t('NPS')}</td><td style={{fontWeight:700}}>{kpis.avgNPS.toFixed(1)}</td><td>{'>'} 8.0</td><td><span className={`chart-card-badge ${kpis.avgNPS>=8?'green':'yellow'}`} style={{display:'inline-block'}}>{kpis.avgNPS>=8?t('OK'):t('ATENÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢O')}</span></td></tr>
              <tr><td>{t('Margem')}</td><td style={{fontWeight:700}}>{kpis.margin.toFixed(1)}%</td><td>{'>'} 20%</td><td><span className={`chart-card-badge ${kpis.margin>=20?'green':'red'}`} style={{display:'inline-block'}}>{kpis.margin>=20?t('OK'):t('CRÃƒÆ’Ã†â€™Ãƒâ€šÃ‚ÂTICO')}</span></td></tr>
            </tbody></table>
          </div></div>
        </div>
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">{t('OcupaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o Semanal (detalhe)')}</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{curve:'smooth' as const,width:3},colors:['#3b82f6'],markers:{size:4,colors:['#3b82f6']},xaxis:{...ct.xaxis,categories:weeklyTrend.map(w=>w.label)},annotations:{yaxis:[{y:75,borderColor:'#22c55e',strokeDashArray:4,label:{text:t('Meta 75%'),style:{color:'#fff',background:'#22c55e'}}}]}}} series={[{name:t('OcupaÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â§ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â£o %'),data:weeklyTrend.map(w=>+w.occupancyRate.toFixed(1))}]} type="line" height={220}/>
          </div></div>
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">{t('Faturamento por Profissional')}</span><span style={{fontSize:10,color:'var(--text-muted)'}}>{t('ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒâ€šÃ‚Â Clique')}</span></div><div className="chart-card-body">
      ...
      {activeTab === 1 && (<>
        ...
        <div className="chart-card" style={{marginBottom: 16}}>
          <div className="chart-card-header">
            <span className="chart-card-title">P1/P2/P3 - Regra de AÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</span>
          ...
          <div className="chart-card-body" style={{ padding: 14 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 10 }}>
              {[
                { key: 'P1', text: 'AÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o em 24h', color: '#ef4444' },
                { key: 'P2', text: 'AÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o em 7 dias', color: '#eab308' },
        ...
        <div className="overview-row">
          ...
          <div className="overview-card"><div className="overview-card-label">OcupaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</div><div className="overview-card-value">{kpis.occupancyRate.toFixed(1)}%</div></div>
        ...
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">02 Taxa de OcupaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o (%)</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{curve:'smooth' as const,width:3},colors:['#3b82f6'],markers:{size:4,colors:['#3b82f6']},xaxis:{...ct.xaxis,categories:agendaWeeks.map(w=>w.label)},annotations:{yaxis:[{y:75,borderColor:'#22c55e',strokeDashArray:4,label:{text:'Meta > 75%',style:{color:'#fff',background:'#22c55e'}}},{y:70,borderColor:'#3b82f6',strokeDashArray:4,label:{text:'P3 70-75',style:{color:'#fff',background:'#3b82f6'}}},{y:60,borderColor:'#eab308',strokeDashArray:4,label:{text:'P2 60-70',style:{color:'#111',background:'#eab308'}}}]}}} series={[{name:'OcupaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o %',data:agendaWeeks.map(w=>+w.occupancyRate.toFixed(1))}]} type="line" height={220}/>
          </div></div>
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">03 ConfirmaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes Realizadas (%)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Combo barra + linha</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{width:[0,3,2],curve:'smooth' as const},plotOptions:{bar:{columnWidth:'45%',borderRadius:4}},colors:['#3b82f6','#22c55e','#64748b'],xaxis:{...ct.xaxis,categories:agendaWeeks.map(w=>w.label)},annotations:{yaxis:[{y:80,borderColor:'#22c55e',strokeDashArray:4,label:{text:'Meta 80%',style:{color:'#fff',background:'#22c55e'}}}]},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:'ConfirmaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes %',type:'column',data:agendaWeeks.map(w=>+w.confirmationRate.toFixed(1))},{name:'Meta %',type:'line',data:agendaWeeks.map(()=>80)},{name:'Base N',type:'line',data:agendaWeeks.map(w=>w.total)}]} type="line" height={220}/>
          </div></div>
        </div>
        <div className="chart-grid">
        ...
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">07 Lead Time do Agendamento (dias)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Linha + mÃƒÆ’Ã‚Â©dia mÃƒÆ’Ã‚Â³vel</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{curve:'smooth' as const,width:[3,2,2]},colors:['#a855f7','#3b82f6','#22c55e'],xaxis:{...ct.xaxis,categories:agendaWeeks.map(w=>w.label)},annotations:{yaxis:[{y:3,borderColor:'#22c55e',strokeDashArray:4,label:{text:'Meta < 3d',style:{color:'#fff',background:'#22c55e'}}},{y:5,borderColor:'#eab308',strokeDashArray:4,label:{text:'P2 > 5d',style:{color:'#111',background:'#eab308'}}},{y:7,borderColor:'#ef4444',strokeDashArray:4,label:{text:'P1 > 7d',style:{color:'#fff',background:'#ef4444'}}}]},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:'Lead time (d)',data:leadTimeSeries},{name:'MÃƒÆ’Ã‚Â©dia mÃƒÆ’Ã‚Â³vel',data:leadTimeAvgSeries},{name:'Meta',data:agendaWeeks.map(()=>3)}]} type="line" height={220}/>
          </div></div>
        ...
        <div className="chart-card">
          <div className="chart-card-header"><span className="chart-card-title">Tabela de Regras (P1/P2/P3)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Status e aÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o por KPI</span></div>
          <div className="chart-card-body" style={{padding:12}}>
            <table className="data-table">
              <thead>
                <tr><th>ID</th><th>KPI</th><th>Valor</th><th>Meta</th><th>Base N</th><th>Status</th><th>AÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o</th></tr>
              </thead>
              <tbody>
                {agendaRules.map((row) => {
      ...
      {activeTab === 2 && (<>
        ...
        <div className="overview-row">
          ...
          <div className="overview-card"><div className="overview-card-label">Receita LÃƒÂ­quida</div><div className="overview-card-value">{fmt(financeCurrent?.net ?? 0)}</div></div>
          <div className="overview-card"><div className="overview-card-label">Margem LÃƒÂ­quida</div><div className="overview-card-value" style={{color:(financeCurrent?.marginPct ?? 0) >= 18 ? 'var(--green)' : 'var(--yellow)'}}>{(financeCurrent?.marginPct ?? 0).toFixed(1)}%</div></div>
          <div className="overview-card"><div className="overview-card-label">Ticket MÃƒÂ©dio</div><div className="overview-card-value">{fmt(financeCurrent?.ticketAvg ?? 0)}</div></div>
        </div>
        <div className="chart-grid">
          ...
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">02 Receita LÃƒÂ­quida (R$)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Bruto - cancelamentos - inadimplÃƒÂªncia - estornos - glosas</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'bar',stacked:true},plotOptions:{bar:{columnWidth:'52%',borderRadius:3}},colors:['#22c55e','#ef4444','#eab308','#8b5cf6','#3b82f6'],xaxis:{...ct.xaxis,categories:financeWeeks.map(w=>w.label)},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:'Receita lÃƒÂ­quida',data:financeWeeks.map(w=>Math.round(w.net))},{name:'Cancelamentos',data:financeWeeks.map(w=>Math.round(w.cancelLoss))},{name:'InadimplÃƒÂªncia',data:financeWeeks.map(w=>Math.round(w.delinquency))},{name:'Estornos',data:financeWeeks.map(w=>Math.round(w.chargebacks))},{name:'Glosas convÃƒÂªnio',data:financeWeeks.map(w=>Math.round(w.conventionGlosas))}]} type="bar" height={220}/>
          </div></div>
        </div>
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">03 Margem LÃƒÂ­quida (%)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Linha + zonas de risco + benchmark</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{curve:'smooth' as const,width:[3,2]},colors:['#a855f7','#94a3b8'],xaxis:{...ct.xaxis,categories:financeWeeks.map(w=>w.label)},annotations:{yaxis:[{y:18,borderColor:'#22c55e',strokeDashArray:4,label:{text:'Meta 18%',style:{color:'#fff',background:'#22c55e'}}},{y:15,borderColor:'#3b82f6',strokeDashArray:4,label:{text:'P3 15-18',style:{color:'#fff',background:'#3b82f6'}}},{y:12,borderColor:'#eab308',strokeDashArray:4,label:{text:'P2 12-15',style:{color:'#111',background:'#eab308'}}}]},fill:{type:'gradient',gradient:{shadeIntensity:1,opacityFrom:0.18,opacityTo:0}},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:'Margem lÃƒÂ­quida %',data:financeWeeks.map(w=>+w.marginPct.toFixed(1))},{name:'Benchmark setor',data:financeWeeks.map(()=>16.5)}]} type="line" height={220}/>
          </div></div>
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">04 Ticket MÃƒÂ©dio (R$)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Benchmark da especialidade</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{curve:'smooth' as const,width:[3,2]},colors:['#ff5a1f','#22c55e'],xaxis:{...ct.xaxis,categories:financeWeeks.map(w=>w.label)},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:'Ticket mÃƒÂ©dio',data:financeWeeks.map(w=>Math.round(w.ticketAvg))},{name:'Benchmark especialidade',data:financeWeeks.map(w=>Math.round(w.ticketBenchmark))}]} type="line" height={220}/>
          </div></div>
        </div>
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">05 InadimplÃƒÂªncia (%)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Bullet chart executivo</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{type:'bar'},plotOptions:{bar:{horizontal:true,barHeight:'45%',borderRadius:4}},colors:['#ef4444'],xaxis:{...ct.xaxis,max:12},annotations:{xaxis:[{x:5,borderColor:'#22c55e',strokeDashArray:4,label:{text:'Meta 5%',style:{color:'#fff',background:'#22c55e'}}},{x:8,borderColor:'#ef4444',strokeDashArray:4,label:{text:'P1 8%',style:{color:'#fff',background:'#ef4444'}}}]},legend:{show:false}}} series={[{name:'InadimplÃƒÂªncia %',data:[+(financeCurrent?.delinquencyPct ?? 0).toFixed(1)]}]} type="bar" height={180}/>
          </div></div>
        ...
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">07 PosiÃƒÂ§ÃƒÂ£o de Caixa (R$)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Linha histÃƒÂ³rica + projeÃƒÂ§ÃƒÂ£o 15d</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{curve:'smooth' as const,width:[3,3]},colors:['#22c55e','#3b82f6'],xaxis:{...ct.xaxis,categories:[...cashProjection.historical.map(p=>p.label), ...cashProjection.projected.map(p=>p.label)]},annotations:{yaxis:[{y:0,borderColor:'#ef4444',strokeDashArray:4,label:{text:'Zero caixa',style:{color:'#fff',background:'#ef4444'}}}]},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:'Caixa histÃƒÂ³rico',data:[...cashProjection.historical.map(p=>Math.round(p.cash)), ...cashProjection.projected.map(()=>null as any)]},{name:'Caixa projetado 15d',data:[...cashProjection.historical.map(()=>null as any), ...cashProjection.projected.map(p=>Math.round(p.cash))]}]} type="line" height={220}/>
          </div></div>
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">Tabela Financeira (P1/P2/P3)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>AÃƒÂ§ÃƒÂµes por regra de negÃƒÂ³cio</span></div><div className="chart-card-body" style={{padding:12}}>
            <table className="data-table"><thead><tr><th>ID</th><th>KPI</th><th>Valor</th><th>Meta</th><th>Base N</th><th>Status</th><th>AÃƒÂ§ÃƒÂ£o</th></tr></thead><tbody>
              ...
              <tr><td>07</td><td>PosiÃƒÂ§ÃƒÂ£o de Caixa (15d)</td><td style={{fontWeight:700}}>{fmt(cashProjection.projected[cashProjection.projected.length-1]?.cash ?? 0)}</td><td>Sempre positivo</td><td>Fluxo previsto</td><td><span className={`chart-card-badge ${badgeForPriority((cashProjection.projected.some(p=>p.cash<0)?'P1':'OK')).className}`} style={{display:'inline-block'}}>{badgeForPriority((cashProjection.projected.some(p=>p.cash<0)?'P1':'OK')).label}</span></td><td>ReforÃƒÂ§ar caixa se projeÃƒÂ§ÃƒÂ£o negativa em qualquer semana</td></tr>
            </tbody></table>
          </div></div>
        </div>
      </>)}
      {/* ===== MARKETING ===== */}
      {activeTab === 3 && (<>
        <div className="section-header"><h2><span className="orange-bar" /> Marketing & CaptaÃ§Ã£o</h2></div>
        <div className="overview-row">
          ...
          <div className="overview-card"><div className="overview-card-label">ConversÃ£o Lead â†’ Agendamento</div><div className="overview-card-value">{(marketingCurrent?.conversion ?? 0).toFixed(1)}%</div></div>
          ...
          <div className="overview-card"><div className="overview-card-label">ROI MÃ©dio por Canal</div><div className="overview-card-value" style={{color:(marketingByChannel.reduce((s,c)=>s+c.roi,0)/Math.max(marketingByChannel.length,1)) > 200 ? 'var(--green)' : 'var(--yellow)'}}>{(marketingByChannel.reduce((s,c)=>s+c.roi,0)/Math.max(marketingByChannel.length,1)).toFixed(0)}%</div></div>
        </div>
        <div className="chart-grid">
          ...
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">02 ConversÃ£o Lead â†’ Agendamento (%)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Meta alvo 30%</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{...ct.chart,type:'line'},stroke:{curve:'smooth' as const,width:[3,2]},colors:['#ff5a1f','#22c55e'],xaxis:{...ct.xaxis,categories:marketingWeeks.map(w=>w.label)},annotations:{yaxis:[{y:30,borderColor:'#22c55e',strokeDashArray:4,label:{text:'Meta 30%',style:{color:'#fff',background:'#22c55e'}}},{y:25,borderColor:'#eab308',strokeDashArray:4,label:{text:'P2 20-25',style:{color:'#111',background:'#eab308'}}},{y:20,borderColor:'#ef4444',strokeDashArray:4,label:{text:'P1 < 20%',style:{color:'#fff',background:'#ef4444'}}}]},legend:{...ct.legend,show:true,position:'bottom' as const}}} series={[{name:'ConversÃ£o %',data:marketingWeeks.map(w=>+w.conversion.toFixed(1))},{name:'Meta',data:marketingWeeks.map(()=>30)}]} type="line" height={220}/>
          </div></div>
        </div>
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">03 No-Show por Canal de Origem (%)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Barra comparativa por canal</span></div><div className="chart-card-body">
            <ReactApexChart options={{...ct,chart:{type:'bar'},plotOptions:{bar:{horizontal:true,distributed:true,barHeight:'60%',borderRadius:4}},colors:['#ef4444','#f97316','#eab308','#3b82f6','#22c55e','#64748b'],xaxis:{...ct.xaxis},annotations:{xaxis:[{x:kpis.noShowRate,borderColor:'#22c55e',strokeDashArray:4,label:{text:'MÃ©dia geral',style:{color:'#fff',background:'#22c55e'}}},{x:25,borderColor:'#eab308',strokeDashArray:4,label:{text:'P2 25%',style:{color:'#111',background:'#eab308'}}},{x:35,borderColor:'#ef4444',strokeDashArray:4,label:{text:'P1 35%',style:{color:'#fff',background:'#ef4444'}}}]},legend:{show:false}}} series={[{name:'No-Show %',data:marketingByChannel.map(c=>({x:c.name,y:+c.noShowRateChannel.toFixed(1)}))}]} type="bar" height={220}/>
          </div></div>
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">04 Custo por Lead â€” CPL (R$)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Benchmark + alerta sem melhora em conversÃ£o</span></div><div className="chart-card-body">
        ...
        <div className="chart-grid">
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">05 ROI por Canal de Marketing (%)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Barra ordenada + semÃ¡foro</span></div><div className="chart-card-body">
          ...
          <div className="chart-card"><div className="chart-card-header"><span className="chart-card-title">Tabela Marketing (P1/P2/P3)</span><span style={{fontSize:10,color:'var(--text-muted)'}}>Alertas e aÃ§Ãµes por KPI</span></div><div className="chart-card-body" style={{padding:12}}>
            <table className="data-table"><thead><tr><th>ID</th><th>KPI</th><th>Valor</th><th>Meta</th><th>Base N</th><th>Status</th><th>AÃ§Ã£o</th></tr></thead><tbody>
      ...
      {/* ===== OPERAÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢O ===== */}
      {activeTab === 4 && (<>
